  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-sm glass-panel rounded-full px-2 py-2 flex justify-between items-center z-50 shadow-2xl animate-in slide-in-from-bottom-10 duration-700 delay-500">
        ...
        <Link
            ...
            className={`flex-1 flex flex-col items-center gap-1 py-2 rounded-full transition-all duration-300 ${isInventory ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
        >
            <div className={`p-2 rounded-full transition-all duration-300 ${isInventory ? 'bg-primary/10 scale-110' : ''}`}>
                <Package size={20} strokeWidth={isInventory ? 2.5 : 2} />
            </div>
        </Link>
